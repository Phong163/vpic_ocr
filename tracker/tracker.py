...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cls):
        ...
        self.score = score
        self.cls = int(cls)  # Lưu class ID (0: plate, 1: number)
        self.tracklet_len = 0
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        self.cls = new_track.cls  # Cập nhật cls
    ...
    def __repr__(self):
        return 'OT_{}_({}-{})_cls_{}'.format(self.track_id, self.start_frame, self.end_frame, self.cls)
...
class BYTETracker(object):
    ...
    def update(self, output_results):
        ...
        # Xử lý đầu vào
        if output_results.shape[1] == 6:  # Bao gồm x1, y1, x2, y2, score, cls
            scores = output_results[:, 4]  # Điểm số
            bboxes = output_results[:, :4]  # Tọa độ hộp (x1, y1, x2, y2)
            classes = output_results[:, 5]  # Class ID
        else:
            output_results = output_results.cpu().numpy()
            scores = output_results[:, 4] * output_results[:, 5]  # Tính score
...
def remove_duplicate_stracks(stracksa, stracksb):
    ...
    for p, q in zip(*pairs):
        ...
        if timep > timeq:
            ...
            dupa.append(p)
    resa = [t for i, t in enumerate(stracksa) if i not in dupb]
